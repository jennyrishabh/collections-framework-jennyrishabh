...
public class customer_signup extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<HTML>");
		out.println("<head><title>Hello from myforms...</title></head><body><br>this is for customr login details..");
		...
		try {
			...
			int r=stmt.executeUpdate("insert into customer values("+"'"+email+"'"+","+"'"+pass+"'"+")");
			if(r==1) {out.println("successfully inserted login details in mysql..");
			response.sendRedirect("/test/customer_home");
			}
