...
public class s_login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs=stmt.executeQuery("select * from specialist order by ID asc");
			...
			if(count==1) {response.sendRedirect("/test/specialist_home");}
			if(count==0) {out.println("INVALID USER.....plz re-enter your details..<br>click this button to redirect");out.println("<button><a href=\"login(specialist).html\">SIGN UP</button> ");}
